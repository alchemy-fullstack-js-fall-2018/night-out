...
const getPlacesUrl = ({ lat, lng }, type) => `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat} ${lng}&radius=1500&type=${type}&key=${apiKey}`;
...
const getPlaces = (location, type) => {
    const url = getPlacesUrl(location, type);
